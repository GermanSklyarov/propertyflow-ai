...
- `GET /properties/saved-searches/lead-analytics`
- `GET /properties/saved-searches/opportunities`
- `GET /properties/saved-searches/alerts/analytics`
...
- `POST /properties/saved-searches/:searchId/lead`
- `GET /properties/saved-searches/:searchId/leads/analytics`
- `GET /properties/saved-searches/:searchId/leads`
- `GET /properties/saved-searches/:searchId/lead-coverage`
...
- `GET /leads/sla`
- `GET /leads/sla/agents`
- `GET /leads/sla/breaches`
- `GET /leads/sla/sources`
- `GET /leads/conversion/agents`
- `GET /leads/conversion/sources`
- `GET /leads/quality`
- `GET /leads/quality/agents`
- `GET /leads/quality/sources`
- `GET /leads/quality-actions`
- `POST /leads/:leadId/quality-actions/assign`
- `POST /leads/:leadId/quality-actions/contact`
- `POST /leads/:leadId/quality-actions/follow-up`
- `POST /leads/:leadId/quality-actions/link-property`
- `POST /leads/:leadId/quality-actions/status`
...
`POST /properties/saved-searches` saves a structured or natural-language search for the current user. Natural-language searches are interpreted into filters before saving, and the snapshot stores `matchCount`, `notificationsEnabled`, the original query, and filters. Agents see their own saved searches; broker/manager/admin roles can review tenant-level saved searches through `GET /properties/saved-searches`. `GET /properties/saved-searches/alerts` returns enabled saved-search alerts with current match counts and top recommendations, which gives notification workers a ready digest shape. `GET /properties/saved-searches/lead-analytics` returns saved-search lead funnel counters and the top saved searches by generated leads. `GET /properties/saved-searches/opportunities` ranks saved searches that have current property matches and need follow-up, includes KPI summary counters, and supports `limit`, `minScore`, and `includeConverted` query parameters for dashboard filtering. `GET /properties/saved-searches/alerts/analytics` returns dashboard counters for enabled alerts and recent digest runs. `POST /properties/saved-searches/alerts/digest-job` enqueues `saved_search.alerts.digest` for the current user's enabled alerts, and the worker stores each completed digest in `saved_search_alert_runs`. `GET /properties/saved-searches/alerts/runs` returns recent digest runs for agent and manager dashboards; `GET /properties/saved-searches/alerts/runs/:runId` returns one run with its saved-search item breakdown. `GET /properties/saved-searches/:searchId/matches` reruns the saved filters and returns current matching listings for recommendation and notification workflows. `GET /properties/saved-searches/:searchId/recommendations` ranks the current matches and explains the top options with reasons and tradeoffs. `POST /properties/saved-searches/:searchId/lead` converts a saved search into a CRM lead with source `saved-search` and validates any selected `propertyId` against current matches. `GET /properties/saved-searches/:searchId/leads` lists CRM leads attributed to that saved search, and `GET /properties/saved-searches/:searchId/leads/analytics` returns status counters plus the latest lead. `GET /properties/saved-searches/:searchId/lead-coverage` shows which current matches already have CRM leads, supports `onlyUncovered`, `limit`, and `sort` (`search-rank`, `uncovered-first`, `latest-lead`) query parameters for follow-up queues, and returns total, filtered, covered, uncovered, returned item count, `hasMore`, and coverage rate. `PATCH /properties/saved-searches/:searchId/notifications` enables or disables saved-search alerts without changing the search filters.
...
`GET /analytics/dashboard` returns tenant dashboard metrics: property counts, lead counts, unassigned leads, lead SLA response breaches, due-soon leads, overdue follow-ups, breached sources, breach rate, health score, lead data-quality issue count, affected lead count, lead data-quality health score, issue buckets, affected lead sources, affected lead agents, leads by source/status, search volume, average search latency, searches by source, top search queries, attributed leads, search-to-lead conversion rate, lead attribution by search source/query, basic conversion rate, AI Concierge adoption, Concierge lead conversion, saved-search volume, saved-search leads, converted saved searches, saved-search lead conversion, saved-search open follow-up opportunities, saved-search follow-up completion rate, saved-search matched properties, saved-search lead-covered matches, saved-search lead coverage rate, feedback quality, recommendation areas, training label coverage, and a `security` block with rejected job enqueue attempts, blocked AI actions, image delete previews, image removals, rejected jobs by name, and blocked AI actions by name.
...
Lead intake supports optional search attribution fields: `attributionSearchEventId`, `attributionSearchQuery`, and `attributionSearchSource`. Lead sources include `website`, `public-api`, `agent`, `ai-chat`, `ai-concierge`, and `saved-search`. `GET /leads` returns the CRM work queue with `status`, `source`, `assignedAgentId`, `unassigned`, `priority`, `followUpDueBefore`, and `limit` filters; agents are scoped to their own assigned leads, while broker, manager, and admin roles can review the tenant queue. `GET /leads/queue-summary` returns the same scoped queue as counters by status, priority, source, assignment state, overdue follow-ups, due-soon follow-ups, and high-priority leads. `GET /leads/sla` returns first-response SLA counters using a four-hour target, including breached leads, due-soon leads, unassigned breaches, overdue follow-ups, average first-response time, and breach source buckets. `GET /leads/sla/agents` returns agent-level SLA performance with assigned totals, open leads, first-response breaches, overdue follow-ups, average first-response time, and breach rate. `GET /leads/sla/sources` returns source-level SLA and funnel performance with totals, open leads, won/lost counts, conversion rate, response breaches, overdue follow-ups, average first-response time, and breach rate. `GET /leads/sla/breaches` returns the ranked SLA worklist with breach types, lead age, overdue follow-up age, and a recommended next action for each affected lead. `GET /leads/conversion/agents` ranks assigned agents by total leads, open/won/lost counts, conversion rate, loss rate, active deal rate, and average time to win. `GET /leads/conversion/sources` ranks lead sources by total leads, open/won/lost counts, conversion rate, loss rate, active deal rate, and average time to win. `GET /leads/quality` returns queue data-quality signals for missing contact info, missing property links, unassigned leads, missing follow-ups, and stale new leads. `GET /leads/quality/agents` ranks assigned agents by affected leads, issue count, issue buckets, affected rate, and queue health score. `GET /leads/quality/sources` ranks lead sources by affected leads, issue count, issue buckets, affected rate, and source health score. `GET /leads/quality-actions` ranks affected leads and returns the next recommended CRM action for each one with a machine-readable `recommendedAction`. `POST /leads/:leadId/quality-actions/contact` applies a missing-contact recommendation by updating email and/or phone, recording audit/realtime events, and optionally adding an activity note. `POST /leads/:leadId/quality-actions/assign` applies an ownership recommendation by assigning the lead to an active agent and optionally adding an activity note. `POST /leads/:leadId/quality-actions/follow-up` applies a recommended follow-up action by scheduling the next touch, optionally raising priority and adding an activity note. `POST /leads/:leadId/quality-actions/link-property` applies a missing-property recommendation by linking the lead to an existing tenant property and optionally adding an activity note. `POST /leads/:leadId/quality-actions/status` applies a stale-lead recommendation by moving the lead through the guarded CRM status workflow and optionally adding an activity note. `PATCH /leads/:leadId/follow-up` updates lead priority and the next follow-up timestamp for agent task planning. `POST /leads/:leadId/notes` and `GET /leads/:leadId/notes` let agents and managers keep a conversation timeline on the lead; agent access is limited to assigned leads. `GET /leads/:leadId/timeline` combines lead creation, assignment, contact updates, follow-up updates, property links, notes, and status changes into one CRM activity feed. `PATCH /leads/:leadId/status` moves a lead through `new`, `contacted`, `qualified`, `lost`, and `won` with guarded transitions, then records audit, realtime, and status-history events for CRM dashboards. `GET /leads/:leadId/status-history` returns the lead status timeline with previous status, new status, actor, role, and timestamp.
