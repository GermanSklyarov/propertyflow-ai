...
export class AnalyticsService {
  ...
  async getDashboard(tenantId: string): Promise<TenantDashboardMetrics> {
    ...
    const leadQualityPenalty =
      metrics.totalLeads > 0 ? Math.round((metrics.leadQualityAffectedLeads / metrics.totalLeads) * 10_000) / 100 : 0;
    ...
    return {
      ...
      leadQualityHealthScore: Math.max(0, Math.round((100 - leadQualityPenalty) * 100) / 100),
