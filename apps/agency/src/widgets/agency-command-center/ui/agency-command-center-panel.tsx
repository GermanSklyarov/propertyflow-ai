...
import styles from "./agency-command-center-panel.module.css";

export function AgencyCommandCenterPanel({ metrics }: { metrics: TenantDashboardMetrics }) {
  return (
    <>
      <section className={styles.kpiGrid} aria-label="Tenant KPIs">
        <KpiCard label="Available listings" note={`${metrics.totalProperties} total inventory`} value={metrics.availableProperties} />
        <KpiCard label="Open leads" note={`${metrics.unassignedLeads} unassigned`} value={metrics.totalLeads} />
        <KpiCard label="Conversion" note={`${metrics.wonLeads} won / ${metrics.lostLeads} lost`} value={formatPercent(metrics.conversionRate)} />
        <KpiCard
...
function KpiCard({ label, note, value }: { label: string; note: string; value: number | string }) {
  return (
      ...
      {items.slice(0, 5).map((item) => (
        <div className={styles.metricRow} key={item.bucket}>
          ...
          <div className={styles.barTrack}>
            <div className={styles.barFill} style={{ width: `${Math.max(8, (item.count / maxCount) * 100)}%` }} />
          </div>
        </div>
      ))}
    </div>
